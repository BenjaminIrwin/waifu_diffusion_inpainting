...
class ImageStore:
    def __init__(self, data_dir: str) -> None:
        ...
        print('ImageStore: Loading conditioning images from ' + self.data_dir)
        self.conditioning_image_files = []
        [self.conditioning_image_files.extend(glob.glob(f'{data_dir}' + '/c*.' + e)) for e in
    ...
    # get image by index
    def get_image_and_conditioning_image(self, idx):
        img = Image.open(self.image_files[idx]).convert('RGB')
        conditioning_image = Image.open(self.conditioning_image_files[idx]).convert('RGB')
        return img, conditioning_image
    ...
    def clean(self) -> None:
        ...
        print('ImageStore: Sorting images and conditioning images')

        clean_images = []
        clean_conditioning_images = []
        ...
        # Create a dictionary that maps conditioning image numbers to filenames
        conditioning_image_dict = {}
        for conditioning_image in self.conditioning_image_files:
            # If conditioning image is valid
            if self.validator(conditioning_image):
                # Extract the number from the filename
                conditioning_image_num = self.extract_input_num(conditioning_image)
                conditioning_image_dict[conditioning_image_num] = conditioning_image

        # Iterate over the keys (numbers) in the image dictionary
        for img_num in image_dict:
            # Check if the number exists in the conditioning image dictionary
            if img_num in conditioning_image_dict:
                # If it does, add the corresponding filenames to the clean lists
                clean_images.append(image_dict[img_num])
                clean_conditioning_images.append(conditioning_image_dict[img_num])

        self.image_files = clean_images
        self.conditioning_image_files = clean_conditioning_images
...
class Img2ImgDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return_dict = {'image_pixel_values': None, 'conditioning_image_pixel_values': None, 'input_ids': None}
        image, conditioning_image = self.store.get_image_and_conditioning_image(idx)
        state = torch.get_rng_state()
        return_dict['image_pixel_values'] = self.transforms(image).to(self.device)
        torch.set_rng_state(state)
        transformed_conditioning_image = self.transforms(conditioning_image).to(self.device)
        return_dict['conditioning_image_pixel_values'] = transformed_conditioning_image
    ...
    def collate_fn(self, examples):
        ...
        image_pixel_values = [example["image_pixel_values"] for example in examples]
        conditioning_image_pixel_values = [example["conditioning_image_pixel_values"] for example in examples]

        image_pixel_values = torch.stack(image_pixel_values).to(memory_format=torch.contiguous_format).float()
        conditioning_image_pixel_values = torch.stack(conditioning_image_pixel_values).to(memory_format=torch.contiguous_format).float()
...
def main():
    ...
    unet = UNet2DConditionModel.from_pretrained(args.model, subfolder='unet', in_channels=8, low_cpu_mem_usage=False,
                                                ignore_mismatched_sizes=True, use_auth_token=args.hf_token)
    ...
    dataset = Img2ImgDataset(store, tokenizer, text_encoder, device, ucg=args.ucg)
        ...
        for epoch in range(args.epochs):
            ...
            for _, batch in enumerate(train_dataloader):
                ...
                with torch.no_grad():
                    ...
                    conditioning_image_latent_dist = vae.encode(
                        batch["conditioning_image_pixel_values"].to(dtype=weight_dtype)).latent_dist
                    latents = latent_dist.sample() * 0.18215
                    conditioning_image_latents = conditioning_image_latent_dist.sample() * 0.18215
                ...
                latent_model_input = torch.cat([noisy_latents, conditioning_image_latents], dim=1)
                                ...
                                with torch.autocast('cuda', enabled=args.fp16):
                                    for _ in range(args.image_log_amount):
                                        if args.enablewandb:
                                            ...
                                            images = \
                                                pipeline(prompt,
                                                         num_inference_steps=args.image_log_inference_steps).images[
                                                    0]
